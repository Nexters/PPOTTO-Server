...
| ANALYSIS-012 | NO_STICKER_SUBJECT | 모든 테마에서 스티커 대상이 없다고 명시적으로 확인됨 |
| ANALYSIS-013 | STICKER_GENERATION_FAILED | 생성 성공이 0개이며 하나 이상의 테마에서 원본 읽기·배경 제거·이미지 처리·저장 등의 오류 발생 |
| ANALYSIS-014 | RESULT_SAVE_FAILED | 생성 결과의 DB 저장 또는 완료 상태 전환 트랜잭션 실패 |
| ANALYSIS-015 | INTERNAL_ERROR | 위 범주에 속하지 않는 처리 오류 또는 장기 미갱신 분석의 만료 |
| ANALYSIS-016 | ANALYSIS_CANCELED | UPLOADING 상태에서 사용자가 취소함 |
| ANALYSIS-017 | CLASSIFICATION_FAILED | 분류 서비스 호출 또는 타임아웃 실패 |
...
- `ANALYSIS-011`은 기존 스티커 재생성 API의 배경 제거·이미지 처리·업로드 실패 코드로 유지됩니다. 초기 분석에서 모든 스티커 생성이 실패한 경우는 `ANALYSIS-013`입니다.
...
- `failedReason`은 진단용 상세 사유입니다. 화면 문구와 복구 동작은 `status`와 `failedCode`를 기준으로 정하고, `ANALYSIS-016`은 생성 오류와 구분합니다.
...
| 404 | ANALYSIS-005 | 분석을 찾을 수 없습니다. | 분석 없음 또는 소유자 불일치 |
| 409 | ANALYSIS-018 | 진행 중인 분석에만 결과 알림을 신청할 수 있습니다. | 분석 상태가 COMPLETED 또는 FAILED (UPLOADING/ANALYZING 중에는 신청 가능) |
...
| failedCode | N | `string \| null` | ANALYSIS-001~017 (006 제외) | 실패 결과 구분 코드. null이면 생략하며, 코드 도입 이전 FAILED 이력에도 없을 수 있음 |
