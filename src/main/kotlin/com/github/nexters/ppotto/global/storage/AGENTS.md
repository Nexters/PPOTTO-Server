...
- `ObjectKeyGenerator` is a pure, stateless class that callers instantiate directly (`PhotoObjectKeys`, `StickerObjectKeys`); it is deliberately not a Spring bean, because nothing injects it and a `@Component` nobody injects is dead wiring. No I/O, no domain imports, no content-type/extension mapping knowledge. It only joins path segments; resolving which extension a given content type maps to is a domain concern (e.g. `analysis`'s `PhotoContentType` enum), not this class's.
...
- `GcsReadUrlIssuerTest` asserts the Valkey key directly (stored value, TTL) and pre-seeds a cache entry to prove the second call does not re-sign. Never assert caching by signing twice and comparing URLs — that only works if the two signatures land in the same second, which is what the old `Thread.sleep(1_100)` was papering over.
