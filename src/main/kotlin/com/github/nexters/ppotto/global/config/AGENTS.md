...
| File | Description |
|------|-------------|
| `SecurityConfig.kt` | Swagger chain (`@Order(0)`, prod Basic), stateless JWT Bearer API chain (`@Order(100)`, non-test), and test/fallback permit-all chain. Login, refresh, health, and swagger are public; all other API requests require authentication. Also defines `CorsConfigurationSource` |
