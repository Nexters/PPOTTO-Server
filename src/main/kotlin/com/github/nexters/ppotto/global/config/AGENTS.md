...
| `AsyncConfig.kt` | `@EnableAsync` plus named virtual-thread-backed `SimpleAsyncTaskExecutor` beans. `analysisCleanupTaskExecutor` is used by `analysis/application/AnalysisCleanupEventListener`; `stickerImageCleanupTaskExecutor` is used by `sticker/application/StickerImageCleanupEventListener` so post-regeneration image deletes stay off the request thread; `analysisPipelineTaskExecutor` is used by `analysis/application/AnalysisPipelineEventListener` so the Gemini pipeline (classification + sticker generation) runs off the `POST /analysis/{id}/start` request thread, keeping the 202 response tied to the `UPLOADING → ANALYZING` transition rather than full pipeline completion |
