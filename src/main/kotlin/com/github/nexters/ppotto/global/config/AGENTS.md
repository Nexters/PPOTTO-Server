...
| `AsyncConfig.kt` | `@EnableAsync` — the project's first async configuration. The `analysisCleanupTaskExecutor` bean is a virtual-thread-backed `SimpleAsyncTaskExecutor`, declared explicitly instead of letting `@Async` fall back to Spring's default `SimpleAsyncTaskExecutor`; `analysis/application/AnalysisCleanupEventListener` references it by name (`AsyncConfig.ANALYSIS_CLEANUP_TASK_EXECUTOR`) to run post-cancel GCS cleanup off the request thread |
