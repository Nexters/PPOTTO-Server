...
|-----------|-------------|
| `application/` | `TermsService` fluent anonymous/authenticated reads, required-term guard, and idempotent agreement pipelines |
| `domain/` | Pure term and agreement models plus `TERM-*` error codes |
| `infrastructure/` | Fluent jOOQ repositories for effective term lookup and idempotent agreement persistence |
