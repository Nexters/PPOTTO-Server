...
data class LoginRequest(
    ...
    fun toCommand(): LoginCommand =
        when (provider) {
            OAuthProvider.KAKAO -> {
                if (name != null) throw InvalidInputException()
                LoginCommand.Kakao(accessToken = accessToken.required())
            }
            OAuthProvider.APPLE -> {
                if (name != null && name.isBlank()) throw InvalidInputException()
                LoginCommand.Apple(
                    identityToken = identityToken.required(),
                    authorizationCode = authorizationCode.required(),
