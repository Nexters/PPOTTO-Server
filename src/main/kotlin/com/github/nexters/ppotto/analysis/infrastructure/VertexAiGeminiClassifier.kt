...
import com.github.nexters.ppotto.analysis.domain.withoutCrossThemeDuplicates
...
class VertexAiGeminiClassifier(
    ...
    override fun classifyAndRecap(photos: List<PhotoRef>): List<ThemeClassification> {
        ...
        val removedCount = classifications.sumOf { it.categorizedPhotoIds.size } - repaired.sumOf { it.categorizedPhotoIds.size }
        if (removedCount > 0) {
            log.warn("여러 테마에 중복 분류된 사진 {}장을 한 테마에만 남겼습니다.", removedCount)
        }
