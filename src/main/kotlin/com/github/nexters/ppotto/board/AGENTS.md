...
| `application/BoardLayoutUpdateCommand.kt` | Application-layer layout command model: changed sticker layouts, created `NewDrawing`s, and deleted drawing ids |
...
- Board create, layout update, and delete acquire the same user-scoped transaction advisory lock before validation and mutation. `BoardRepository.lockCommandsByUserId` renders `pg_advisory_xact_lock` through a jOOQ template and refuses to run outside a transaction (`TransactionSynchronizationManager.isActualTransactionActive`), because autocommit would release the lock the moment it was taken. The key is the literal `board-user:{userId}`; `BoardRepositoryTest` repeats that literal on a second JDBC connection and asserts `pg_try_advisory_xact_lock` is refused while the transaction runs and granted after it ends, so a guard without a real lock fails there.
...
- `BoardLayoutService` owns every layout rejection: duplicate created or deleted drawing ids, the same id in both lists, duplicate sticker ids, non-finite sticker coordinates and `scale <= 0` are `COMMON-001`, while a created id that already belongs to another board and a deleted id that is not active on this board are `BOARD-001`. Every rejection happens before the sticker port is called, so a rejected request leaves nothing written; `BoardLayoutServiceTest` asserts that per reason.
