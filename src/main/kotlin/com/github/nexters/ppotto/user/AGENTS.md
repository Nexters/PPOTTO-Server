...
- Missing auth adapters fail closed: provider-account or session revoke aborts withdrawal. A provider that answers 4xx is not a failure — Apple rejecting an already-revoked token means the goal state holds, so `AppleOAuthClient.revoke` swallows `HttpClientErrorException` only and lets 5xx or network errors abort as before.
