...
- **`ck_users_social_identity_complete` is `NOT VALID`, which exempts existing rows only until something updates them.** Postgres re-checks a `NOT VALID` constraint on every later `INSERT` and `UPDATE`, so `V20260807103000__add_user_name.sql`'s blanket `UPDATE users SET name = '홍길동'` aborts on any row still missing provider, provider user id, or email. Both migrations applied cleanly because no incomplete legacy row was left by then. `UserMigrationTest` runs the two migrations in order in one nested path and pins both halves: the abort with a legacy row left incomplete, and the `홍길동` backfill once its identity is filled in. Any future table-wide `UPDATE` on `users` carries the same trap — backfill the identity first.
...
- **Withdrawal order is: read the account, revoke the provider account over HTTP, write the anonymized row, then revoke the service refresh-token session.** The provider revoke runs before any write, so a revoke failure aborts the withdrawal with nothing changed (fail-closed). The read plus the guarded single-row `UPDATE ... WHERE deleted_at IS NULL` need no transaction — a concurrent withdrawal makes the update return no row and the caller gets `USER-001`. Session revoke goes through `TransactionSynchronizationManager` so it runs `afterCommit` if a caller ever wraps this in a transaction, and inline otherwise. Neither the HTTP call nor the Redis call may move back inside a DB transaction. `UserServiceTest` pins every step of that order: a revoker that fails aborts the withdrawal with the row still active and no session revoked, a revoker that reads the row observes it still active, a `TransactionTemplate` caller sees no session revoke until commit and none at all on rollback, and two concurrent withdrawals produce exactly one success and one `USER-001`.
...
- Every provider deletes its object-storage objects before its own rows, and each provider commits its own transaction. A partial failure therefore leaves the user soft-deleted with the work partly done; each port is idempotent, so the next batch run re-runs the whole sequence and converges. The user row is hard-deleted only after all four ports return. `WithdrawnUserDataDeletionIntegrationTest` drives this with a data-bearing user and a board port that throws: stickers and analysis are gone, boards, term agreements and the user row survive, and the next run with the real ports finishes the job.
- Cleanup deletes nothing in `user_device_tokens`. There is no FK and no deletion port for it, so a hard-deleted user's device-token rows stay behind as inert leftovers; `findFcmTokensByUserId` is only ever reached with a live user id. Adding a fifth port is the fix if that ever has to change.
