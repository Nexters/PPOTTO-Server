...
| `config/gcs.yml` | `gcs.bucket` / `gcs.credentials-path` / `gcs.signed-url-expiration-minutes` from `${GCS_*}` |
| `db/migration/` | Flyway migrations: `V{yyyyMMddHHmmss}__{description}.sql` (timestamp version). The first migration enables the pgvector extension; the second creates `users`/`boards`/`images` and the shared `set_updated_at()` trigger function; the third adds `images.original_file_name` |
