...
class PhotoRepositoryTest(
    ...
    userRepository: UserRepository,
        ...
        Given("빈 아이템 목록으로 배치 저장을 호출하면") {
            ...
            When("저장을 수행하면") {
                val saved = photoRepository.saveAll(analysis.id, board.id.value, emptyList())
        ...
        Given("PENDING 상태의 Photo가 저장된 상태에서") {
            ...
            When("기대 상태(PENDING)를 걸고 COMPLETED로 배치 갱신하면") {
                val saved =
                    photoRepository.saveAll(analysis.id, board.id.value, listOf(PhotoCreate(PhotoContentType.JPEG, Instant.now())))
        ...
        Given("서로 다른 분석에 완료 사진과 대기 사진이 저장된 상태에서") {
            ...
            When("다른 분석의 완료 사진 id로 소유 수를 세면") {
                val count = photoRepository.countOwnedByAnalysis(analysis.id, board.id.value, listOf(otherPhoto.id))
