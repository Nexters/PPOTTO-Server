        ...
        Given("부모 스팬이 없는 비동기 경로에서") {
            When("LLM 호출을 trace로 감싸면") {
                ...
                Then("스펙에서 폐기된 중복 키는 넣지 않는다") {
                    ...
                    data.keys.contains("gen_ai.system") shouldBe false
                    data.keys.contains("gen_ai.request.messages") shouldBe false
                    data.keys.contains("gen_ai.response.text") shouldBe false
                }

                Then("프롬프트와 응답 본문을 현행 스펙 키로 담는다") {
                    val data =
                        captured
                            .single()
                            .contexts.trace
