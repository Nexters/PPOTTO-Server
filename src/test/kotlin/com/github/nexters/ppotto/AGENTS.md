...
| `global/openapi/OpenApiDocumentationTest.kt` | Verifies the actual `/v3/api-docs` output: metadata, endpoint contracts, exactly one `X-API-Version` parameter per operation, JSON success-response schemas with examples, non-null required fields, request constraints, domain error examples, production `ObjectMapper` values, schema field names/examples, and authentication modes |
