...
| `support/UserTestFixtures.kt` | `UserRepository.saveTestUser()` extension that inserts a schema-valid Kakao test user, plus the transitional `User.rawId` accessor marking call sites that still feed raw `UUID` APIs during the typed-id migration |
