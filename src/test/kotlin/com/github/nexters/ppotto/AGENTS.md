...
| `support/IntegrationTest.kt` | Base class: `@SpringBootTest` + `@ActiveProfiles("test")` + every shared test import — `TestcontainersConfiguration`, `ObjectStorageTestConfiguration`, and `NotificationTestConfig` (the FCM push dummy). Extend this for integration tests |
...
| `global/storage/GcsReadUrlIssuerTest.kt` | Read signed URL reuse through Valkey, cache expiration, and direct-signing fallback when Valkey is unavailable |
| `global/logging/RequestLoggingFilterTest.kt` | Request logging contract tests: headers are logged, `Authorization` is masked case-insensitively, and `/actuator/**` stays skipped |
| `global/observability/SentryHttpPayloadEndToEndTest.kt` | Real-Tomcat (`RANDOM_PORT`) end-to-end check with Sentry actually enabled and a recording `BeforeSendTransactionCallback` / `BeforeSendLogCallback`. Proves what MockMvc cannot: bodies are captured when the controller reads them, when the security chain 401s before reading, and on unmapped paths; every attribute value is a scalar (arrays are rejected by ingest) and headers land as individual masked keys; and INFO logs really reach Sentry Logs. Three separate production regressions got through MockMvc-only tests before this existed |
| `global/observability/SentryHttpPayloadFilterTest.kt` | Drives the filter against a real recording `Sentry.init` and a `MockFilterChain`. Proves the response body still reaches the client after `ContentCachingResponseWrapper` buffering, that request/response headers and bodies land on the transaction span with secrets masked, that a body the app never read is still captured, and that no active span means no buffering |
