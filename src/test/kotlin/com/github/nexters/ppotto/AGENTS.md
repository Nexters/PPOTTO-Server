...
| `support/UserTestFixtures.kt` | `UserRepository.saveTestUser()` extension that inserts a schema-valid Kakao test user. Tests pass `user.id` (typed) to application APIs and `user.id.value` only at raw boundaries (security principal, jOOQ DSL, analysis-internal repositories) |
...
| `global/openapi/OpenApiDocumentationTest.kt` | Verifies the actual `/v3/api-docs` output: metadata, the endpoint contracts declared on interfaces, exactly one `X-API-Version` parameter per operation, success-response envelope schemas exposed together with per-scenario examples, domain error code failure examples, example values matching the production `ObjectMapper` settings (null omission, `Instant` UTC notation, `Double` notation), schema field names/examples, and authentication modes |
