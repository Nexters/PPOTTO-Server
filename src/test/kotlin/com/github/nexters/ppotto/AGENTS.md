...
| `auth/` | OAuth HTTP timeout/config validation, deterministic JWT tamper detection, active-user refresh validation, Bearer security tests including optional-auth terms behavior, and cross-domain 가입 tests: 기본 보드 실패 rollback, 약관 조회 실패 rollback, 로그인 트랜잭션 경계, 동시 가입 중복 방지 |
