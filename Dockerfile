...
RUN --mount=type=bind,from=build,source=/workspace/src/test/resources/dummy-gcs-key.json,target=/tmp/dummy-gcs-key.json,readonly \
    ...
    ANALYSIS_PIPELINE_MAX_CONCURRENT_RUNS=10 ANALYSIS_PIPELINE_RESUME_LIMIT=20 \
    ANALYSIS_STALE_CLEANUP_ENABLED=false ANALYSIS_STALE_CLEANUP_TIMEOUT_MINUTES=60 \
    ANALYSIS_STALE_CLEANUP_BATCH_SIZE=100 ANALYSIS_STALE_CLEANUP_CRON="0 */10 * * * *" \
    ...
    VERTEX_AI_CLASSIFY_TIMEOUT_MS=60000 VERTEX_AI_VERIFY_TIMEOUT_MS=30000 \
