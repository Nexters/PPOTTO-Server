...
| File | Description |
|------|-------------|
| `workflows/ci.yml` | CI: Gradle build, tests, lint, and Kover XML on PR/push to `dev`; posts a coverage comment on PRs via mi-kas/kover-report |
...
- Repo settings: squash merge only, branch auto-delete. `dev` is the default and only long-lived branch; the "dev 보호" ruleset exists but is disabled.
